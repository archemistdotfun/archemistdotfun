...
interface ITestQuoteAsset {
    function balanceOf(address account) external view returns (uint256);
    function approve(address spender, uint256 amount) external returns (bool);
    function transfer(address to, uint256 amount) external returns (bool);
}

contract SmokeErc20PairBaseSepolia is Script {
    ...
    function run() external returns (address tokenAddress, PoolId poolId) {
        ...
        ITestQuoteAsset quote = ITestQuoteAsset(vm.envAddress("MOCK_USDT"));

        vm.startBroadcast(adminKey);
        if (!quote.transfer(trader, FUND_AMOUNT)) revert TransferFailed();
        vm.stopBroadcast();

        bytes32 salt = keccak256(abi.encode("mock-usdt-live", block.timestamp));
        address predicted = launcher.computeTokenAddress(salt, "USDT Pair Smoke", "USDTARC", trader);
        ...
        bool tokenIsCurrency0 = predicted < address(quote);
        ...
        uint256 creatorFee = locker.claimable(trader, address(quote));
        locker.claim(address(quote), trader);
        vm.stopBroadcast();

        console2.log("Mock USDT quote", address(quote));
        ...
        console2.log("trader mock USDT balance", quote.balanceOf(trader));
    }
}
